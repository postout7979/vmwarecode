...
let AppComponent = class AppComponent {
    constructor(authenticationService, cmbuService) {
...
/* harmony import */ var ngx_owl_carousel_o__WEBPACK_IMPORTED_MODULE_40__ = __webpack_require__(/*! ngx-owl-carousel-o */ "./node_modules/ngx-owl-carousel-o/fesm2015/ngx-owl-carousel-o.js");
...
/* harmony import */ var ngx_cookie_service__WEBPACK_IMPORTED_MODULE_43__ = __webpack_require__(/*! ngx-cookie-service */ "./node_modules/ngx-cookie-service/ngx-cookie-service.js");
...
let AddToPlaylistComponent = class AddToPlaylistComponent {
    constructor(cmbuService) {
        this.cmbuService = cmbuService;
    ...
    getPlayLists() {
        this.cmbuService.getPlayLists().subscribe(data => {
            this.userPlaylists = data.data;
            if (this.userPlaylists.length == 0) {
                this.isPlaylistToDisplay = false;
            }
            else {
                this.isPlaylistToDisplay = true;
            }
        });
    }
    changeModalStatus() {
    ...
    onSubmit() {
        ...
        let playListDescription = this.playListDescription;
    ...
    createPlayList(playListName) {
        ...
        this.cmbuService.addToPlayList(this.playListName, this.playListDescription).subscribe(data => {
            if (data.status) {
                this.playListName = '';
                this.playListDescription = '';
                this.getPlayLists();
                this.addToPlayList(data.slug);
                this.cmbuService.getLibrarySidebarMenuCount().subscribe(data => {
                    this.cmbuService.librarySidebarFavoriteCount = data.favorite_count;
                    this.cmbuService.librarySidebarPlaylistCount = data.playlist_count;
                });
            }
            else {
                this.isError = true;
    ...
    addToPlayList(playListName) {
        this.playListName = playListName;
        this.cmbuService.addAssetsToPlayList(this.playListName, this.assetToBeAdded).subscribe(data => {
            // if(data.status){
            this.playListName = '';
            this.playListDescription = '';
            this.addToPlaylistPopup = false;
            let self = this;
            this.userPlaylists.filter(function (x) {
...
AddToPlaylistComponent.ctorParameters = () => [
...
let AdminPlaylistComponent = class AdminPlaylistComponent {
    constructor(router, playlistsService, paginationService, cmbuService) {
        this.router = router;
    ...
    getPlaylists() {
        this.playlistsService.getPlaylists(this.keyword, 'admin', this.currentPage, this.pageSize).subscribe(data => {
            this.playlists = data.data;
            this.paginationService.displayPaginationResult(data.pagination);
        });
    }
    filterUsecasesByKeyword(keyword) {
        this.keyword = keyword;
        this.getPlaylists();
    }
    getPaginationAssets(currentPage) {
        this.currentPage = currentPage;
        this.getPlaylists();
    }
    getPlaylistsByPageSize(pageSize) {
...
AdminPlaylistComponent.ctorParameters = () => [
...
let AssetDetailsComponent = class AssetDetailsComponent {
    constructor(cmbuService, route, sanitizer, authenticationService, router, _location) {
        this.cmbuService = cmbuService;
        this.route = route;
    ...
    ngOnInit() {
        this.route.paramMap.subscribe(data => {
            let assetID = data.get('assetID');
            this.cmbuService.getSingleAssetDetails(assetID).subscribe(data => {
                if (!data.data[0]) {
                    this.router.navigate(['/**'], { skipLocationChange: true });
                    return false;
                }
                this.AssetID = data.data[0].assetId;
                this.assetType = data.data[0].assetType.name;
                this.assetTitle = data.data[0].assetTitle;
                this.assetDescription = data.data[0].assetDescription;
                this.isDislike = data.data[0].useractions.isDislike;
                this.isLike = data.data[0].useractions.isLike;
                this.likePercentage = data.data[0].useractions.likePercentage;
                this.isFavourite = data.data[0].useractions.isFavourite;
                this.dislikePercentage = data.data[0].useractions.dislikePercentage;
                //this.iframeLink = data.data[0].assetLink;
                this.iframeLink = this.sanitizer.bypassSecurityTrustResourceUrl(data.data[0].assetLink);
                this.howToDemo = data.data[0].howToDemo;
                this.howToDemoType = data.data[0].howToDemoType;
                this.howToDemoLink = this.sanitizer.bypassSecurityTrustResourceUrl(data.data[0].howToDemoLink);
                if (this.authenticationService.isLoggedIn()) {
                    this.cmbuService.addAssetsToRecentlyViewed(assetID).subscribe(data => {
                    });
                }
            });
        });
    }
    likeAsset(assetID, event, assetTitle) {
        let isUserLoggedIn = this.getUserAccessToken();
        if (isUserLoggedIn != '') {
            this.cmbuService.likeAssets(assetID).subscribe(data => {
                this.changeLikeAndDisLikeStatus(data);
    ...
    addToFavourite(assetID, event) {
        let isUserLoggedIn = this.getUserAccessToken();
        if (isUserLoggedIn != '') {
            this.cmbuService.addToFavourite(assetID).subscribe(data => {
    ...
    disLikeAssets(assetID, event, assetTitle) {
        let isUserLoggedIn = this.getUserAccessToken();
        if (isUserLoggedIn != '') {
            this.cmbuService.disLikeAssets(assetID).subscribe(data => {
                this.changeLikeAndDisLikeStatus(data);
    ...
    changeLikeAndDisLikeStatus(data) {
...
let AssetItemBoxComponent = class AssetItemBoxComponent {
    constructor(authenticationService, cmbuService, router, renderer, elRef) {
        this.authenticationService = authenticationService;
        this.cmbuService = cmbuService;
        this.router = router;
        ...
        this.renderer.listen('window', 'click', (e) => {
            let relatedAssetElement = this.elRef.nativeElement.querySelectorAll('.card .related-assets');
            let tempShowRelatedAssets = false;
            ...
            let relatedAssetContainerElement = this.elRef.nativeElement.querySelectorAll('.card .related-assets-container');
    ...
    addToFavourite(assetID, event) {
        ...
        if (isUserLoggedIn != '') {
            this.cmbuService.addToFavourite(assetID).subscribe(data => {
                if (this.router.url == '/library/favorites') {
                    this.getFavoriteAssets.emit();
                }
                this.cmbuService.getLibrarySidebarMenuCount().subscribe(data => {
                    this.cmbuService.librarySidebarFavoriteCount = data.favorite_count;
                    this.cmbuService.librarySidebarPlaylistCount = data.playlist_count;
    ...
    openRelatedAssets(index, assetID) {
        this.relatedAssets = [];
        ...
        this.currentAssetsId = assetID;
        this.cmbuService.getRelatedAssets(assetID).subscribe(results => {
            this.relatedAssets = results.data;
            setTimeout(function () {
                let relatedAssetPosts = document.querySelectorAll('.related-posts .container');
    ...
    close(event) {
        let scrollUpBox = document.querySelectorAll('.related-posts .container');
        let scrollUp = this.elRef.nativeElement.querySelectorAll('.related-posts .showBtn');
    ...
    hide(event) {
        let assetDescriptionBox = document.querySelectorAll('.new-card .assetDescriptionBox');
        let assetDescriptionBoxExpand = this.elRef.nativeElement.querySelectorAll('.new-card .showBtn');
...
AssetItemBoxComponent.ctorParameters = () => [
...
let AssetViewComponent = class AssetViewComponent {
    constructor() { }
    ngOnInit() {
    }
};
...
let BreadcrumbComponent = class BreadcrumbComponent {
    ...
    replaceHyphenFromUrl(urlsegment) {
...
let CreatePlaylistComponent = class CreatePlaylistComponent {
    constructor(cmbuService, router) {
        this.cmbuService = cmbuService;
        this.router = router;
        this.modalStatus = new _angular_core__WEBPACK_IMPORTED_MODULE_1__["EventEmitter"]();
        ...
        if (!this.isRenameEvent) {
            this.cmbuService.addToPlayList(this.playListName, this.playListDescription).subscribe(data => {
                if (data.status) {
                    ...
                    this.router.navigateByUrl('/', { skipLocationChange: true }).then(() => this.router.navigateByUrl('library/user-playlists'));
                }
                else {
            ...
            this.cmbuService.renamePlayList(this.playListName, this.playListID, this.playListDescription).subscribe(data => {
                if (data.status) {
                    this.playListName = '';
                    this.createPlaylistPopup = false;
                    this.router.navigateByUrl('/', { skipLocationChange: true }).then(() => this.router.navigateByUrl('library/user-playlists'));
...
let DiscoverSliderComponent = class DiscoverSliderComponent {
    constructor(authenticationService, router, cmbuService, elRef, renderer) {
        this.authenticationService = authenticationService;
        this.router = router;
        this.cmbuService = cmbuService;
        this.elRef = elRef;
    ...
    openRelatedAssets(index, assetID, event) {
        this.relatedAssets = [];
        let owlitms = this.elRef.nativeElement.querySelectorAll('.owl-item');
...
DiscoverSliderComponent.ctorParameters = () => [
    { type: src_app_services_authentication_service__WEBPACK_IMPORTED_MODULE_2__["AuthenticationService"] },
...
let EssentialSliderComponent = class EssentialSliderComponent {
    constructor(authenticationService, cmbuService, router, route, renderer, elRef) {
        this.authenticationService = authenticationService;
        this.cmbuService = cmbuService;
        this.router = router;
        this.route = route;
    ...
    hideShowMoreBtn(length, useCaseAsset) {
        if (length <= 1) {
...
EssentialSliderComponent.ctorParameters = () => [
...
let FaqComponent = class FaqComponent {
    ...
    ngOnInit() {
        this.faqService.getFaqDetails().subscribe(data => {
            this.faqDetails = data.data;
        });
    }
...
let FavoritesComponent = class FavoritesComponent {
    constructor(cmbuService, paginationService) {
        this.cmbuService = cmbuService;
        this.paginationService = paginationService;
        this.assetsList = [];
        this.keyword = '';
    ...
    getFavoriteAssets() {
        this.cmbuService.getFavoriteAssets(this.keyword, this.currentPage, this.pageSize).subscribe(data => {
...
let FeedbackComponent = class FeedbackComponent {
    ...
    setFeedbaclValue(feedbackPoint) {
        this.feedbackPoint = feedbackPoint;
    }
    onSubmit() {
        this.submitted = true;
        this.feedbackService.submitFeedbackForm(Object.values(this.formValues)).subscribe(data => {
            this.isError = data.status;
            this.isFeedbackResponse = true;
            this.feedbackResponse = data.message;
            this.submitted = false;
            this.formFields = JSON.parse(JSON.stringify(this.formFields));
            this.formValues = JSON.parse(JSON.stringify(this.tempFormValues));
        });
    }
    getFormFields() {
        this.feedbackService.getFeedbackFormFields().subscribe(result => {
            if (result.status == true) {
                this.formFields = result.data;
                this.formFields.forEach(formField => {
                    this.formValues[formField.id] = {
                        'id': formField.id,
                        'value': null,
                        'option_id': null,
                        'option_ids': {}
                ...
                this.tempFormValues = JSON.parse(JSON.stringify(this.formValues));
            }
        });
    }
};
    ...
    constructor(router) {
        ...
        router.events.subscribe(event => {
            if (event instanceof _angular_router__WEBPACK_IMPORTED_MODULE_2__["NavigationEnd"]) {
                let urlDetails = event.url;
                let urlSegment = urlDetails.split('/');
...
let HeaderComponent = class HeaderComponent {
    constructor(service, route, authenticationService, router, notificationService) {
        ...
        router.events.subscribe(event => {
            if (event instanceof _angular_router__WEBPACK_IMPORTED_MODULE_3__["NavigationEnd"]) {
                if (location.pathname != '/') {
                    this.currentPathname = '?returnUrl=' + location.pathname;
                }
                let urlDetails = event.url;
                let urlSegment = urlDetails.split('/');
    ...
    checkNewNotification() {
        if (this.authenticationService.isLoggedIn()) {
            this.notificationService.checkNewNotifications().subscribe(result => {
                this.service.isAnyNewNotification = result.isAnyNewNotification;
                this.notificationService.isSubscribed = result.isSubscribed;
            });
        }
    }
    ...
    redirectPage() {
        if (this.searchKeywordHeader && this.searchKeywordHeader.trim() != "") {
            ...
            let searPageURL = '/search/all-assets/' + this.searchKeywordHeader;
...
let HomeComponent = class HomeComponent {
    ...
    getLatestAssets() {
        this.service.getLatestAssets('', 1, 10).subscribe(result => {
            if (result.status) {
                let totalCount = parseInt(result.pagination.totalCount);
                if ((totalCount < 10) && ((totalCount % 2) != 0)) {
                    result.assets.pop();
                    this.assetDetails = result.assets;
                }
                else {
                    this.assetDetails = result.assets;
                }
    ...
    getLatestUseCases() {
        this.service.getLatestUseCasesCategories().subscribe(data => {
            this.useCaseAssets = data.data;
            if (this.useCaseAssets.length == 0) {
                this.isEssentialToDisplay = false;
            }
            else {
                this.isEssentialToDisplay = true;
            }
...
let LibrarySidebarComponent = class LibrarySidebarComponent {
    constructor(router, cookieService, cmbuService) {
        this.router = router;
...
LibrarySidebarComponent.ctorParameters = () => [
    { type: _angular_router__WEBPACK_IMPORTED_MODULE_2__["Router"] },
...
let LibraryComponent = class LibraryComponent {
    constructor(router, route, cmbuService, authenticationService) {
        this.router = router;
        this.route = route;
        this.cmbuService = cmbuService;
        this.authenticationService = authenticationService;
    ...
    ngOnInit() {
        ...
        this.router.events.subscribe((e) => {
            if (e instanceof _angular_router__WEBPACK_IMPORTED_MODULE_2__["NavigationStart"]) {
                this.libraryTab(e.url);
            }
        });
        if (this.authenticationService.isLoggedIn()) {
            var self = this;
            setTimeout(function () {
                self.cmbuService.getLibrarySidebarMenuCount().subscribe(data => {
...
let LoginComponent = class LoginComponent {
    constructor(cmbuService, authenticationService, route, router) {
        this.cmbuService = cmbuService;
        this.authenticationService = authenticationService;
        this.route = route;
        this.router = router;
    ...
    ngOnInit() {
        this.route.paramMap.subscribe(data => {
            this.assetID = data.get('assetID');
            this.assetAction = data.get('assetAction');
        });
        this.returnUrl = this.route.snapshot.queryParams['returnUrl'] || '/';
        if (this.authenticationService.isLoggedIn()) {
            if (this.assetAction == 'like') {
                this.cmbuService.likeAssets(this.assetID).subscribe(data => {
                });
            }
            else if (this.assetAction == 'favourite') {
                this.cmbuService.addToFavourite(this.assetID).subscribe(data => {
                });
            }
...
let ManagePlaylistComponent = class ManagePlaylistComponent {
    constructor(playlistsService, router, route, cmbuService) {
        this.playlistsService = playlistsService;
        this.router = router;
        this.route = route;
        this.cmbuService = cmbuService;
    ...
    ngOnInit() {
        this.route.params.subscribe((params) => {
            this.getAssetsFromPlaylists(params['playlist-slug']);
            this.playlistSlug = params['playlist-slug'];
        });
    }
    getAssetsFromPlaylists(playlistSlug) {
        this.playlistsService.getAssetsFromPlaylists(playlistSlug, this.keyowrd, this.currentPage, this.pageSize).subscribe(data => {
            if (!data.name) {
                this.router.navigate(['/**'], { skipLocationChange: true });
                return false;
            }
            this.playlist = data;
            this.playlistAssets = data.assets;
        });
    }
    removeAssetsFromPlaylists(playlistSlug, assetId) {
        this.playlistsService.removeAssetsFromPlaylists(playlistSlug, assetId).subscribe(data => {
            if (data.status) {
                const currentAsset = this.playlistAssets.find(x => x.assetId === assetId);
                const index = this.playlistAssets.indexOf(currentAsset);
                if (index !== -1) {
                    this.playlistAssets.splice(index, 1);
                }
            }
        });
    }
    deletePlaylist(playlistId) {
        this.playlistsService.deletePlaylist(playlistId).subscribe(data => {
            if (data.status) {
                this.cmbuService.getLibrarySidebarMenuCount().subscribe(data => {
                    this.cmbuService.librarySidebarFavoriteCount = data.favorite_count;
                    this.cmbuService.librarySidebarPlaylistCount = data.playlist_count;
                });
                this.router.navigate(['/library/user-playlists']);
            }
        });
    }
    filterByKeyword(keyword) {
...
ManagePlaylistComponent.ctorParameters = () => [
...
let NotificationComponent = class NotificationComponent {
    constructor(notificationService, paginationservice, location, cmbuService) {
        this.notificationService = notificationService;
        this.paginationservice = paginationservice;
    ...
    getAllNotifications(currentPage, pageSize) {
        this.notificationService.getAllNotifications(currentPage, pageSize).subscribe(data => {
            this.allNotifications = data.data;
            this.totalAssetCount = data.pagination.totalCount;
            this.displayPaginationResult(data.pagination);
        });
    }
    getHours(createdTime) {
        ...
        let diffInSec = Math.abs(Date.parse(this.currentDate) - Date.parse(createdTime)) / 1000;
        let diffHrs = Math.floor(diffInSec / 3600) % 24;
        let diffMins = Math.floor(diffInSec / 60) % 60;
        let diffSec = diffInSec % 60;
        let result = '';
        if (diffHrs > 0) {
    ...
    removeNotification(motificationID) {
        this.notificationService.removeNotification(motificationID).subscribe(data => {
            this.getAllNotifications(this.currentPage, this.totalNumberOfAssetsPerPage);
        });
    }
    getPaginationAssets(currentPage) {
        if (currentPage) {
            let tempTotalNumberOfAssetsPerPage = 0;
    ...
    displayPaginationResult(paginationDetails) {
        this.paginationservice.displayPaginationResult(paginationDetails);
    }
    markAsRead(notificationID, notificationLink, notificationLinkType) {
        this.notificationService.viewNotification(notificationID).subscribe(data => {
            this.cmbuService.isAnyNewNotification = data.isAnyNewNotification;
    ...
    markAllAsRead() {
        this.notificationService.viewNotification('all').subscribe(data => {
            this.getAllNotifications(this.currentPage, this.totalNumberOfAssetsPerPage);
            this.checkNewNotification.emit();
            if (data.status) {
                this.cmbuService.isAnyNewNotification = false;
            }
        });
    }
    subscribe() {
        this.notificationService.isSubscribed = !this.notificationService.isSubscribed;
        this.notificationService.subscribe().subscribe(result => {
            console.log(result);
        });
    }
};
...
let PaginationButtonComponent = class PaginationButtonComponent {
    ...
    keydownEvent(event) {
        ...
        if (event.key === "Enter") {
            ...
            this.paginationFunction = setTimeout(function () {
                self.triggerPagination(event);
            }, 1000);
        }
    }
    triggerPagination(event) {
        let currentPageInput = parseInt(event.target.value);
        if (currentPageInput > 0 && currentPageInput <= this.totalNoOfPages) {
            this.navigatePagination(currentPageInput);
        }
    }
};
PaginationButtonComponent.ctorParameters = () => [
...
let PaginationCountComponent = class PaginationCountComponent {
    constructor() {
        this.PageSize = new _angular_core__WEBPACK_IMPORTED_MODULE_1__["EventEmitter"]();
        this.selectedPageSize = "10";
    }
    ngOnInit() {
        let selectedPageSize = localStorage.getItem("SelectedPageSize");
        if (selectedPageSize) {
            this.selectedPageSize = selectedPageSize;
            this.PageSize.emit(this.selectedPageSize);
        }
    }
    getAssetsByPageSize(event) {
...
let PaginationResultComponent = class PaginationResultComponent {
    constructor(paginationService) {
        this.paginationService = paginationService;
    }
    ngOnInit() {
...
PaginationResultComponent.ctorParameters = () => [
    { type: src_app_services_paginationservice_service__WEBPACK_IMPORTED_MODULE_2__["PaginationserviceService"] }
];
tslib__WEBPACK_IMPORTED_MODULE_0__["__decorate"]([
    Object(_angular_core__WEBPACK_IMPORTED_MODULE_1__["Input"])()
...
let PlaylistItemComponent = class PlaylistItemComponent {
    constructor(playlistService, router, cmbuService, authenticationService) {
    ...
    savePlaylist(playlistId) {
        ...
        this.playlistService.savePlaylist(playlistId).subscribe(data => {
            if (data.status) {
                const currentPlaylist = this.playlists.find(x => x.id === playlistId);
                currentPlaylist.savedPlaylist = true;
            }
        });
    }
    removePlaylist(playlistId) {
        this.playlistService.removePlaylist(playlistId).subscribe(data => {
            if (data.status) {
                const currentPlaylist = this.playlists.find(x => x.id === playlistId);
                currentPlaylist.savedPlaylist = false;
                this.getSavedPlaylists.emit();
            }
        });
    }
    deletePlaylist(playlistId) {
    ...
    isshowPlaylistPopup(playlistSlug) {
        this.playlistSlug = playlistSlug;
        this.isRenameEvent = true;
        this.showPlaylistPopup = true;
        this.playlistService.getAssetsFromPlaylists(playlistSlug, this.keyword, this.currentPage, this.pageSize).subscribe(data => {
...
let PlaylistViewComponent = class PlaylistViewComponent {
    constructor(playlistsService, route, paginationService, router) {
        this.playlistsService = playlistsService;
        this.route = route;
        this.paginationService = paginationService;
        this.router = router;
    ...
    ngOnInit() {
        this.route.paramMap.subscribe(data => {
            this.playlistSlug = data.get('playlistSlug');
            this.getAssetsFromPlaylists();
        });
    }
    getAssetsFromPlaylists() {
        this.playlistsService.getAssetsFromPlaylists(this.playlistSlug, this.keyword, this.currentPage, this.pageSize).subscribe(data => {
            if (!data.name) {
                this.router.navigate(['/**'], { skipLocationChange: true });
                return false;
            }
...
PlaylistViewComponent.ctorParameters = () => [
    { type: src_app_services_playlists_service__WEBPACK_IMPORTED_MODULE_2__["PlaylistsService"] },
    { type: _angular_router__WEBPACK_IMPORTED_MODULE_3__["ActivatedRoute"] },
    { type: src_app_services_paginationservice_service__WEBPACK_IMPORTED_MODULE_4__["PaginationserviceService"] },
    { type: _angular_router__WEBPACK_IMPORTED_MODULE_3__["Router"] }
...
let ProductSidebarComponent = class ProductSidebarComponent {
    ...
    filterAssetsByCategory(productCategory) {
...
let ProductComponent = class ProductComponent {
    constructor(service, route, router, paginationservice) {
        this.service = service;
        this.route = route;
        this.router = router;
        this.paginationservice = paginationservice;
        ...
        router.events.subscribe((val) => {
            if (val instanceof _angular_router__WEBPACK_IMPORTED_MODULE_3__["ActivationEnd"]) {
                this.productSlug = val.snapshot.paramMap.get('product-slug');
                this.categorySlug = '';
                this.assetKeyword = '';
                this.currentPage = 1;
            ...
            if (val instanceof _angular_router__WEBPACK_IMPORTED_MODULE_3__["NavigationEnd"]) {
                this.getAssetCountByProductSlug(this.productSlug);
            }
        });
    }
    ngOnInit() {
        this.route.paramMap.subscribe((params) => {
            this.productSlug = params.get('product-slug');
        });
    }
    getAssetCountByProductSlug(productSlug) {
        this.service.getAssetCountByProductSlug(productSlug).subscribe(data => {
            this.productCategories = data.categories;
            this.allProductCount = data.totalCount;
        });
    }
    getAssetsByProductSlug(productSlug, categorySlug, assetKeyword, currentPage, totalNumberOfAssetsPerPage) {
        this.service.getAssetsByProductSlug(productSlug, categorySlug, assetKeyword, currentPage, totalNumberOfAssetsPerPage).subscribe(data => {
            if (!data.productDetails) {
                this.router.navigate(['/**'], { skipLocationChange: true });
                return false;
            }
            this.productAssets = data.assets;
            this.productDetails = data.productDetails;
            this.totalAssetCount = data.totalCount;
            this.getNoOfPages(this.totalNumberOfAssetsPerPage);
            this.displayPaginationResult(data.pagination);
        });
    }
    filteredAssetsByCategorySlug(categorySlug) {
        this.assetKeyword = '';
    ...
    filterAssetsByKeyword(assetKeyword) {
    ...
    getNoOfPages(totalNumberOfAssetsPerPage) {
...
let RecentlyViewedComponent = class RecentlyViewedComponent {
    constructor(cmbuService, paginationService) {
        this.cmbuService = cmbuService;
        this.paginationService = paginationService;
        this.assetsList = [];
    ...
    getRecentlyViewedAssets() {
        this.cmbuService.getRecentlyViewedAssets(this.keyword, this.currentPage, this.pageSize).subscribe(data => {
            this.assetsList = data.data;
            this.paginationService.displayPaginationResult(data.pagination);
        });
    }
...
RecentlyViewedComponent.ctorParameters = () => [
    { type: src_app_services_cmbu_service__WEBPACK_IMPORTED_MODULE_2__["CmbuService"] },
    { type: src_app_services_paginationservice_service__WEBPACK_IMPORTED_MODULE_3__["PaginationserviceService"] }
];
...
let SearchAssistantBannerComponent = class SearchAssistantBannerComponent {
    constructor(router, cmbuService, route) {
        this.router = router;
        this.cmbuService = cmbuService;
        this.route = route;
        this.isLive = false;
        ...
        this.currentSlug = '';
        this.route.paramMap.subscribe(data => {
            this.currentSlug = data.get('categorySlug');
        });
    }
    ngOnInit() {
        // this.categorySlug = 'live';
        this.cmbuService.getAllAssetCategories().subscribe(data => {
            this.assetCategories = data.data;
            this.categorySlug = 'all-assets'; //data.data[0].count;
        });
    }
    getCategorySlug(event) {
    ...
    redirectPage() {
        if (!this.searchKeyword || this.searchKeyword.trim() == "") {
            ...
            let searPageURL = '/search/' + this.categorySlug + '/' + this.searchKeyword;
            this.router.navigateByUrl(searPageURL);
        }
    }
};
SearchAssistantBannerComponent.ctorParameters = () => [
    { type: _angular_router__WEBPACK_IMPORTED_MODULE_2__["Router"] },
    { type: src_app_services_cmbu_service__WEBPACK_IMPORTED_MODULE_3__["CmbuService"] },
    { type: _angular_router__WEBPACK_IMPORTED_MODULE_2__["ActivatedRoute"] }
];
...
let SearchResultButtonComponent = class SearchResultButtonComponent {
    constructor(router) {
        this.router = router;
        ...
        router.events.subscribe((val) => {
            if (val instanceof _angular_router__WEBPACK_IMPORTED_MODULE_2__["ActivationEnd"]) {
                this.searchKeyword = val.snapshot.paramMap.get('searchKeyword');
            }
        });
    }
    ngOnInit() {
    }
    redirectPage(categorySlug) {
        let searPageURL = '';
        this.categorySlug = categorySlug;
        this.router.navigateByUrl('/search/' + this.categorySlug + '/' + this.searchKeyword);
    }
};
SearchResultButtonComponent.ctorParameters = () => [
    { type: _angular_router__WEBPACK_IMPORTED_MODULE_2__["Router"] }
];
tslib__WEBPACK_IMPORTED_MODULE_0__["__decorate"]([
    Object(_angular_core__WEBPACK_IMPORTED_MODULE_1__["Input"])()
...
let SearchComponent = class SearchComponent {
    constructor(service, router, paginationservice) {
        this.service = service;
        this.router = router;
        this.paginationservice = paginationservice;
        this.productSlug = '';
        ...
        router.events.subscribe((val) => {
            if (val instanceof _angular_router__WEBPACK_IMPORTED_MODULE_3__["ActivationEnd"]) {
                this.categorySlug = val.snapshot.paramMap.get('categorySlug');
                if (this.categorySlug == 'all-assets') {
                    this.categorySlug = '';
                }
                this.searchKeyword = val.snapshot.paramMap.get('searchKeyword');
            }
    ...
    getAssetsByCategorySlug(categorySlug, assetKeyword, currentPage, pageSize) {
        this.categorySlug = categorySlug;
        this.assetKeyword = assetKeyword;
        let t1 = new Date().getTime();
        this.service.getAssetsByCategorySlug(this.categorySlug, this.assetKeyword, currentPage, pageSize).subscribe(data => {
            let t2 = new Date().getTime();
            this.productAssets = data.assets;
            this.assetCategories = data.categories;
            this.assetCategoryResultCount = data.categories.filter(x => x.slug == this.categorySlug)[0].count;
...
let TopRatedComponent = class TopRatedComponent {
    constructor(router, cmbuService, paginationservice) {
        this.router = router;
        this.cmbuService = cmbuService;
        this.paginationservice = paginationservice;
        this.assetsList = [];
    ...
    getRopRatedAssets() {
        this.cmbuService.getTopRatedAssets(this.keyword, this.currentPage, this.pageSize).subscribe(data => {
            this.assetsList = data.data;
            this.paginationservice.displayPaginationResult(data.pagination);
        });
    }
...
let UnsubscribeComponent = class UnsubscribeComponent {
    constructor(notificatonService) {
        this.notificatonService = notificatonService;
        this.message = '';
    }
    ngOnInit() {
        this.notificatonService.unSubscribe().subscribe(result => {
            this.message = result.message;
        });
    }
};
UnsubscribeComponent.ctorParameters = () => [
    { type: src_app_services_notification_service__WEBPACK_IMPORTED_MODULE_2__["NotificationService"] }
];
...
let UseCasesComponent = class UseCasesComponent {
    constructor(usecasesService, router, paginationService, cmbuService) {
    ...
    getUsecases() {
        this.usecasesService.getUsecases(this.useCaseKeyword, this.currentPage, this.pageSize).subscribe(data => {
            this.usecasesList = data.data;
            this.paginationService.displayPaginationResult(data.pagination);
        });
    }
    filterUsecasesByKeyword(keyword) {
    ...
    getUsecasesByPageSize(pageSize) {
        this.pageSize = pageSize;
        this.getUsecases();
    }
};
UseCasesComponent.ctorParameters = () => [
...
let UsecaseItemComponent = class UsecaseItemComponent {
    constructor() { }
    ngOnInit() {
    }
    viewUseCase(usecaseDetails) {
...
let UsecaseViewComponent = class UsecaseViewComponent {
    constructor(service, route, paginationService, router) {
        this.service = service;
        this.route = route;
        this.paginationService = paginationService;
        this.router = router;
    ...
    ngOnInit() {
        this.route.paramMap.subscribe(data => {
            this.usecaseSlug = data.get('usecaseslug');
            this.getUseCasesAssets();
        });
    }
    getUseCasesAssets() {
        this.service.getUsecasesAssets(this.usecaseSlug, this.keyword, this.currentPage, this.pageSize).subscribe(data => {
            if (!data.name) {
                this.router.navigate(['/**'], { skipLocationChange: true });
                return false;
            }
            this.usercaseDetails = data;
            this.productAssets = data.assets;
            this.paginationService.displayPaginationResult(data.pagination);
        });
    }
...
UsecaseViewComponent.ctorParameters = () => [
...
let UserPlaylistComponent = class UserPlaylistComponent {
    constructor(playlistsService, paginationService) {
        this.playlistsService = playlistsService;
        this.paginationService = paginationService;
        this.playlists = [];
        this.savedPlaylists = [];
    ...
    getPlaylists() {
        this.playlistsService.getPlaylists(this.keyword, 'users', this.currentPage, this.pageSize).subscribe(data => {
            this.playlists = data.data;
            this.paginationService.displayPaginationResult(data.pagination);
        });
    }
    getSavedPlaylists() {
        this.playlistsService.getSavedPlaylists(this.savedPlaylistsKeyword, this.savedPlaylistCurrentPage, this.savedPlaylistPageSize).subscribe(data => {
            this.savedPlaylists = data.data;
            this.paginationService.displayPaginationResult(data.pagination);
        });
    }
    getPlaylistsByPageSize(pageSize) {
    ...
    getSavedPlaylistsByPageSize(pageSize) {
    ...
    getPaginationUserPlaylists(currentPage) {
        this.currentPage = currentPage;
        this.getPlaylists();
    }
    getPaginationSavedPlaylists(currentPage) {
...
UserPlaylistComponent.ctorParameters = () => [
    { type: src_app_services_playlists_service__WEBPACK_IMPORTED_MODULE_2__["PlaylistsService"] },
    { type: src_app_services_paginationservice_service__WEBPACK_IMPORTED_MODULE_3__["PaginationserviceService"] }
];
...
let WhatsNewComponent = class WhatsNewComponent {
    constructor(cmbuService, router, paginationservice) {
        this.cmbuService = cmbuService;
        this.router = router;
        this.paginationservice = paginationservice;
        this.newAssets = [];
    ...
    getLatestAssets() {
        this.cmbuService.getLatestAssets(this.keyword, this.currentPage, this.totalNumberOfAssetsPerPage).subscribe(data => {
            this.newAssets = data.assets;
            this.currentPage = 1;
            this.totalAssetCount = data.totalCount;
            this.getNoOfPages(this.totalNumberOfAssetsPerPage);
            this.paginationservice.displayPaginationResult(data.pagination);
...
let SpaceCutterDirective = class SpaceCutterDirective {
    constructor(el) {
        this.el = el;
    }
    onInput() {
...
SpaceCutterDirective.ctorParameters = () => [
    { type: _angular_core__WEBPACK_IMPORTED_MODULE_1__["ElementRef"] }
];
tslib__WEBPACK_IMPORTED_MODULE_0__["__decorate"]([
    Object(_angular_core__WEBPACK_IMPORTED_MODULE_1__["HostListener"])('input')
...
let AuthenticationService = class AuthenticationService {
    ...
    isLoggedIn() {
        let matomoUser = this.cookieService.get('matomoUser');
...
let CmbuService = class CmbuService {
    ...
    getAssetsByCategorySlug(categorySlug, searchKeyword, currentPage, pageSize) {
        this.checkUserLoginStatus();
    ...
    getLatestAssets(keyword = '', currentPage, pageSize) {
        this.checkUserLoginStatus();
    ...
    getTopRatedAssets(keyword = '', currentPage, pageSize) {
        this.checkUserLoginStatus();
    ...
    getRecentlyViewedAssets(keyword = '', currentPage, pageSize) {
        this.checkUserLoginStatus();
    ...
    getFavoriteAssets(keyword = '', currentPage, pageSize) {
        this.checkUserLoginStatus();
    ...
    addToPlayList(playListName, description) {
        this.checkUserLoginStatus();
        let postData = { 'playlistName': playListName, 'description': description };
    ...
    renamePlayList(playListName, playListID, playlistDescription) {
        this.checkUserLoginStatus();
        let postData = { 'playlistId': playListID, 'playlistName': playListName, 'description': playlistDescription };
    ...
    addAssetsToPlayList(playListName, assetId) {
        let postData = { 'playlistSlug': playListName, 'assetId': assetId };
        this.checkUserLoginStatus();
    ...
    addAssetsToRecentlyViewed(assetID) {
        this.checkUserLoginStatus();
    ...
    checkUserLoginStatus() {
        let accessToken = this.authenticationService.getUserAccessToken();
...
let FeedbackService = class FeedbackService {
    ...
    submitFeedback(feedBackText, feedbackPoint) {
        this.checkUserLoginStatus();
        let postData = { 'type': feedbackPoint, 'message': feedBackText };
...
let PlaylistsService = class PlaylistsService {
    ...
    getPlaylists(keyword = '', playlistType, currentPage, pageSize) {
    ...
    getSavedPlaylists(keyword = '', currentPage, pageSize) {
    ...
    getAssetsFromPlaylists(playlistSlug, keyowrd, currentPage, pageSize) {
...
let UsecasesService = class UsecasesService {
    ...
    getUsecases(keyword, currentPage, pageSize) {
    ...
    getUsecasesAssets(usecaseSlug, keyword, currentPage, pageSize) {
...
UsecasesService.ctorParameters = () => [
    { type: _angular_common_http__WEBPACK_IMPORTED_MODULE_3__["HttpClient"] },
    { type: _authentication_service__WEBPACK_IMPORTED_MODULE_4__["AuthenticationService"] }
];
